...
export function detectFramework(projectRoot: string): FrameworkConfig {
    ...
    if (deps['next']) { return { name: 'Next.js', command: 'npm run dev', port: 3000 }; }
    if (deps['@angular/core']) { return { name: 'Angular', command: 'npm run start', port: 4200 }; }
    if (deps['@vue/cli-service'] || scripts['serve']) { return { name: 'Vue CLI', command: 'npm run serve', port: 8080 }; }
    if (deps['nuxt'] || deps['nuxt3']) { return { name: 'Nuxt', command: 'npm run dev', port: 3000 }; }
    if (deps['vite'] || deps['@vitejs/plugin-react'] || deps['@vitejs/plugin-vue']) { return { name: 'Vite', command: 'npm run dev', port: 5173 }; }
    if (deps['react-scripts']) { return { name: 'React', command: 'npm start', port: 3000 }; }
    if (deps['@sveltejs/kit']) { return { name: 'SvelteKit', command: 'npm run dev', port: 5173 }; }
    if (deps['@remix-run/react']) { return { name: 'Remix', command: 'npm run dev', port: 3000 }; }
    if (scripts['dev']) { return { name: 'Unknown', command: 'npm run dev', port: 3000 }; }
    if (scripts['start']) { return { name: 'Unknown', command: 'npm start', port: 3000 }; }
...
export function waitForServer(port: number, timeoutMs = 60000): Promise<boolean> {
...
function injectScriptTag(indexHtmlPath: string, scriptSrc: string): void {
    if (!fs.existsSync(indexHtmlPath)) { return; }
    let html = fs.readFileSync(indexHtmlPath, 'utf8');
    if (html.includes(SS_MARKER)) { return; } // already injected
...
function removeScriptTag(indexHtmlPath: string): void {
    if (!fs.existsSync(indexHtmlPath)) { return; }
    let html = fs.readFileSync(indexHtmlPath, 'utf8');
    if (!html.includes(SS_MARKER)) { return; }
...
let _activeIndexHtml: string | undefined;
let _activeProjectRoot: string | undefined;

let _onBrowserMessage: ((msg: any) => void) | undefined;
let _onBrowserPanelOpen: (() => void) | undefined;

export function setBrowserMessageHandler(handler: (msg: any) => void): void {
    _onBrowserMessage = handler;
}

export function setBrowserPanelOpenHandler(handler: () => void): void {
    _onBrowserPanelOpen = handler;
}

export function postToBrowser(msg: object): void {
    browserPanel?.webview.postMessage(msg);
}

export function getBrowserPanel(): vscode.WebviewPanel | undefined {
    return browserPanel;
}
...
export async function launchDevServer(
    ...
    if (!fs.existsSync(selectorSrc)) {
        vscode.window.showErrorMessage(
            'SnapStak: ss-tracker.js not found. Ensure media/copywriter/ss-tracker.js is included in the extension package.'
        );
        // Still open the browser panel — tracker just won't be available
    }

    const copyTracker = (dest: string): void => {
        try { if (fs.existsSync(selectorSrc)) { fs.copyFileSync(selectorSrc, dest); } }
        catch (e: any) { console.error('[SnapStak] copyTracker failed:', e.message); }
    };
    ...
    await vscode.window.withProgress(
        ...
        async (progress) => {
            ...
            if (!ready) {
                vscode.window.showErrorMessage(
                    `SnapStak: Dev server did not start on port ${framework.port} within 60 seconds.`
                );
                return;
            }
...
function openBrowserPanel(
    ...
    setTimeout(() => { if (_onBrowserPanelOpen) { _onBrowserPanelOpen(); } }, 300);

    browserPanel.webview.onDidReceiveMessage((msg: any) => {
        if (msg.command === 'openExternal') {
            vscode.env.openExternal(vscode.Uri.parse(msg.url));
            return;
        }
        if (_onBrowserMessage) { _onBrowserMessage(msg); }
    });
    ...
    browserPanel.onDidDispose(() => {
        ...
        if (_activeProjectRoot) {
            ...
            try { if (fs.existsSync(tempCopy)) { fs.unlinkSync(tempCopy); } } catch { /* silent */ }
...
function getBrowserHtml(initialUrl: string, port: number): string {
    return /* html */`
<!DOCTYPE html>
<html lang="en">
<head>
