...
export function setDrawingCanvasSidebarView(view: vscode.WebviewView | undefined): void {
    _sidebarView = view;
    if (view) {
        view.webview.onDidReceiveMessage((msg: any) => {
            ...
            function openImageFile(callback: (src: string, w: number, h: number, fileName: string) => void) {
            ...
            if (msg.command === 'fetchLucideTags') {
                const serverUrl = vscode.workspace.getConfiguration('snapstak').get<string>('serverUrl', 'http://localhost:3001');
...
export function postToDrawingCanvas(msg: object): void {
    _panel?.webview.postMessage(msg);
}

export function openDrawingCanvasPanel(context: vscode.ExtensionContext): void {
    ...
    _panel.webview.onDidReceiveMessage((msg: any) => {
        ...
        if (msg.command === 'rectSelected') { _sidebarView?.webview.postMessage(msg); }
        if (msg.command === 'rectResized') { _sidebarView?.webview.postMessage(msg); }
        if (msg.command === 'polySelected') { _sidebarView?.webview.postMessage(msg); }
        if (msg.command === 'polyResized') { _sidebarView?.webview.postMessage(msg); }
    });
