...
import { SnapStakClient, CopywriterBlock } from '../snapstakClient';
import { buildInjectionMap, writeInjectedContent } from '../copywriter/injection';
import { postToBrowser, getBrowserPanel } from '../devRunner';

let currentPanel: vscode.WebviewPanel | undefined;
let _storedContext: vscode.ExtensionContext | undefined;
let _storedClient: any | undefined;
let _pendingMsg: any | undefined;

let _onCopywriterMessage: ((msg: any) => void) | undefined;

export function setCopywriterMessageHandler(handler: (msg: any) => void): void {
    _onCopywriterMessage = handler;
}

export function initCopywriter(context: vscode.ExtensionContext, client: any): void {
    _storedContext = context;
    _storedClient = client;
}

export function openCopywriterPanel(
    context: vscode.ExtensionContext,
    client: SnapStakClient,
    projectRoot?: string
): void {
    _storedContext = context;
    _storedClient = client;
    ...
    currentPanel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.type) {
            ...
            case 'SHOW_BLOCK_PROPERTIES': {
                if (_onCopywriterMessage) { _onCopywriterMessage(msg); }
                break;
            }
            ...
            case 'clearTracker': {
                postToBrowser({ command: 'ss_clear' });
                // Tracker was destroyed by _finish() — restart it so user can reselect.
                setTimeout(() => {
                    postToBrowser({ command: 'ss_start' });
                    // Restore group mode if that was the active mode
                    setTimeout(() => { postToBrowser({ command: 'ss_set_content_mode', mode: msg.contentMode || 'text' }); }, 100);
                }, 100);
                break;
            }
            ...
            case 'GENERATE_HEADING': {
                try {
                    const heading = await client.copywriterGenerateHeading(
                        msg.blockId, msg.originalHeading, msg.headingTag,
                        msg.wordCount, msg.businessDescription, msg.keywords
                    );
                    currentPanel?.webview.postMessage({
            ...
            case 'GENERATE_ALL_HEADINGS': {
                try {
                    const headings = await client.copywriterGenerateAllHeadings(
                        msg.blocks, msg.businessDescription
                    );
            ...
            case 'REWRITE_BODY_BATCH': {
                const results: Record<string, Array<{
                    text: string; wordCount: number; needsReview: boolean;
                }>> = {};
                try {
                    await Promise.all(
                        (msg.blocks as CopywriterBlock[])
                            .filter(b => b.bodyText.length > 0)
                            .map(async (block) => {
                                const confirmedHeading = msg.confirmedHeadings[block.blockId];
                                results[block.blockId] = await Promise.all(
                                    block.bodyText.map(node =>
                                        client.copywriterRewriteBody(
                                            block.blockId, confirmedHeading,
                                            node.text, node.wordCount, msg.businessDescription
                                        )
                                    )
                                );
                            })
            ...
            case 'REWRITE_BODY_SINGLE': {
                try {
                    const result = await client.copywriterRewriteBody(
                        msg.blockId, msg.confirmedHeading, msg.originalText,
                        msg.wordCountTarget, msg.businessDescription, msg.retryContext
                    );
                    currentPanel?.webview.postMessage({
            ...
            case 'INJECT_CONTENT': {
                ...
                try {
                    const injectionMap = buildInjectionMap(msg.blocks, msg.state);
                    await writeInjectedContent(projectRoot, injectionMap);
                    currentPanel?.webview.postMessage({ type: 'INJECTION_COMPLETE' });
                    vscode.window.showInformationMessage(
                        'SnapStak Copywriter: Content injected successfully.'
                    );
                } catch (err: any) {
                    vscode.window.showErrorMessage(
                        `SnapStak Copywriter: Injection failed. ${err.message}`
                    );
                }
                break;
            }
...
export function forwardToCopywriter(msg: any): void {
    if (currentPanel) {
