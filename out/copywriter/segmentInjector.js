...
exports.injectSegmentIds = injectSegmentIds;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const FRAMEWORK_ATTR_MAP = {
    'React': {
        class: 'className',
        for: 'htmlFor',
        tabindex: 'tabIndex',
...
function scanDir(dir, projectRoot) {
    const priorityFiles = [];
    const otherFiles = [];
    function walk(currentDir, isPriority) {
        let entries;
        try {
        ...
        for (const entry of entries) {
            if (entry.isDirectory()) {
                ...
                // Skip injected files
                if (entry.name === 'ss-tracker.js' || entry.name === 'contentSelector.js') {
                    continue;
                }
...
// ─────────────────────────────────────────────────────────────
function translateAttributes(openingTag, attrMap) {
...
// ─────────────────────────────────────────────────────────────
function buildSearchVariants(openingTag, attrMap) {
    const variants = [];
...
// ─────────────────────────────────────────────────────────────
function injectIntoTag(source, matchIndex, segmentId) {
...
function alreadyHasSegmentId(source, matchIndex) {
...
// MAIN INJECTOR
// ─────────────────────────────────────────────────────────────
async function injectSegmentIds(projectRoot, elements, registry, framework = 'Unknown') {
    const result = {
        success: false,
        injectedCount: 0,
        skippedCount: 0,
    ...
    for (const [relPath, updatedContent] of modifiedFiles.entries()) {
        ...
        try {
            ...
            result.errors.push(`Failed to write ${relPath}: ${e.message}`);
        }
    }
    result.success = result.injectedCount > 0;
    return result;
}
//# sourceMappingURL=segmentInjector.js.map